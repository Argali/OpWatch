...
|---|---|---|
| Frontend (React) | GitHub Pages | `https://argali.github.io/OpWatch` |
| Backend (Express) | Render (free tier) | `https://OpWatch-backend.onrender.com` |

### First-time Render setup

1. Go to [render.com](https://render.com) → New → **Blueprint** → connect the repo
2. Render reads `render.yaml` and creates the `OpWatch-backend` service automatically
3. In the Render dashboard, go to the service → **Environment** and set these manually:

| Variable | Value |
|---|---|
| `FRONTEND_URL` | `https://argali.github.io/OpWatch` |
...
5. Verify: `curl https://OpWatch-backend.onrender.com/health` → `{"status":"ok","uptime":...}`
...
| Secret | Value |
|---|---|
| `VITE_API_URL` | `https://OpWatch-backend.onrender.com/api` |
...
```bash
# 1. Backend alive?
curl https://OpWatch-backend.onrender.com/health

# 2. CORS correct? (should return data, not a CORS error)
curl -H "Origin: https://argali.github.io" https://OpWatch-backend.onrender.com/health

# 3. Open the app and log in
# https://argali.github.io/OpWatch
```
